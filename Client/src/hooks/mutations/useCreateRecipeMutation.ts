...
export function useCreateRecipeMutation(): UseMutationResult<CreateRecipeResponse, Error, CreateRecipeRequest> {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (request: CreateRecipeRequest) => {
      const response = await fetch('/api/recipes', { method: "POST", headers: { 'Accept': 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify(request) });
      if (!response.ok) {
